...
> Architecture Decision Records for the AI Travel Planner: a multi-agent, multi-city planner that grounds itineraries in real POIs, weather, and routing.
> Every decision is anchored to the service's target NFRs: design for ~1M MAU / ~500 peak concurrent (demonstrate ~50 via k6); quality-first ≤ **$0.30/itinerary**; multi-city (≤5 cities, ≤10 days); full-itinerary p95 45s; 99.5% SLO; GDPR-aligned hygiene; ≤ ~$50/mo idle dev.

## At-a-glance summary (all 22)

| # | Decision | Options considered | Picked | Why (short) | Prod-grade? | Reversibility |
|---|---|---|---|---|---|---|
...
| 2 | Vector DB | Qdrant / pgvector / Pinecone / Milvus / Weaviate | **Qdrant** (~1024-dim, hybrid) | Filtered hybrid search at scale; multilingual multi-city | Yes | HARD once loaded |
| 3 | Agent architecture | ReAct / plan-execute graph / **multi-agent** | **Multi-agent** (coordinator + per-city workers + critic) | Clean multi-city decomposition, parallel fan-out, central critic | Yes | Moderate-Hard |
| 4 | LLM provider & tiering | single frontier / **tiered+fallback gateway** / open-weight | **Gateway**: cheap→mid→frontier tiers; OpenAI primary, Anthropic/Groq fallback rungs | Quality where it counts + cost control + SLO resilience | Yes | Easy-Moderate |
| 5 | Embedding model | **Voyage** / Cohere / OpenAI-3-large / bge-m3 | **1024-dim embeddings** (OpenAI default; Voyage `voyage-3` same dim) | Compact vectors; dimension pinned so the engine stays swappable | Yes | HARD (re-embed) |
| 6 | Orchestration | **LangGraph** / thin loop / CrewAI | **LangGraph** (supervisor/worker; tools framework-agnostic) | Durable checkpoints + human-in-the-loop + parallel nodes | Yes | Moderate |
...
| 8 | Frontend & streaming | **Next.js** / Svelte / keep Streamlit | **Next.js** + Tailwind/shadcn + SSE + MapLibre (Streamlit=dev-only) | The live trace is the product; first-class SSE streaming | Yes | Moderate |
| 9 | Auth | Clerk / Cognito / **Auth0** | **Auth0** (OIDC/JWT/Orgs; Cognito AWS-native alt) | Mature, enterprise SSO, recognizable | Yes | Moderate |
| 10 | Caching | **Redis** / Memcached / none | **Redis**: embedding + tool-result caches; + provider prompt cache | Biggest cost lever; reuse as broker | Yes | Easy |
| 11 | Queue & async | ARQ / **Celery** / SQS / Temporal | **Celery + Redis broker** (RabbitMQ alt; KEDA autoscale) | Mature, battle-tested; reuses Redis | Yes | Moderate |
| 12 | Inference serving | **hosted APIs** / vLLM / SageMaker | **Hosted APIs behind gateway** (OpenAI primary; Bedrock alt); vLLM future | No idle GPU; full prompt-cache+tool parity | Yes | Easy |
| 13 | Observability | **OTel+Langfuse** / LangSmith / CloudWatch | **OTel → Prom/Grafana + Loki; Langfuse** (LangSmith alt) | Per-step agent tracing; vendor-neutral collection; logs join traces | Yes | Easy-Moderate |
| 14 | Cloud | **AWS** / GCP / Azure | **AWS** (EKS, Aurora, ElastiCache, S3, ECR, Secrets Mgr, CloudFront, R53, ALB, WAF) | Deepest service catalog; covers every need here | Yes | Hard |
| 15 | Containers/IaC | **EKS** / ECS Fargate / Fly | **Docker multi-stage → EKS**; Terraform + Helm + ArgoCD | Full control over scheduling/autoscaling at the target scale | Yes | Moderate-Hard |
| 16 | CI/CD | **GitHub Actions** / GitLab / Jenkins | **GH Actions + ArgoCD + Argo Rollouts**; mandatory eval gate | Industry default; eval gate = production | Yes | Easy-Moderate |
| 17 | Secrets/config | **Secrets Mgr+ESO** / k8s Secrets / Vault | **AWS Secrets Manager + ESO + pydantic-settings (fail-fast)** | Out of repo, rotatable; fail-fast config | Yes | Easy-Moderate |
| 18 | Security/threat | defense-in-depth | **Agentic threat model**: untrusted retrieval, least-privilege tool authz, structured tool I/O, read-only tools, PII redaction, WAF/TLS/mTLS, audit, injection suite | Tools take actions, so the tool layer is the risk center | Yes | Moderate |
| 19 | Evaluation | layered | **RAGAS + agent-trajectory + LLM-judge** on versioned golden sets; CI gate; online sampling + drift; A/B; adversarial | Blocks quality + budget regressions before they ship | Yes | Easy |
| 20 | Cost controls | defense-in-depth | **Tiering + caching; per-run and per-sub-agent hard caps; per-tenant quotas + rate limits; spend alerts + kill switch** | Holds the per-itinerary ceiling under multi-agent fan-out | Yes | Easy |
| 21 | Failure/degradation | per-failure matrix | **Circuit breakers + retries/backoff + timeouts; partial results; honest no-answer; backpressure + bulkheads; cross-provider fallback** | An SLO on external providers needs an explicit fallback per dependency | Yes | Easy-Moderate |
| 22 | Repo structure | mono vs poly | **uv monorepo**: apps/(web, api, worker) + packages/(agents, tools, core, retrieval, eval) + infra + tests + docs | Shared baseline, atomic cross-package changes, one CI | Yes | Moderate |

Reversibility = how costly the decision is to change later; it drives how much deliberation each one got.
...
- **Reasoning:** Multi-city work decomposes cleanly along city boundaries: each city is independent, so per-city workers fan out in parallel (which is what makes the p95 target reachable) and a single critic can validate the merged plan. The single-city path is built first and remains a working fallback if the coordinator is bypassed.
- **Trade-offs:** Multi-agent reliability is genuinely harder to guarantee, and per-city fan-out puts pressure on the $0.30 ceiling — accepted only WITH the containment contract above and a trajectory-eval bar to catch regressions.
...
- **Decision:** Model **gateway** with three tiers routed by step difficulty — CHEAP (`gpt-4o-mini`) for routine sub-steps, MID (`gpt-4o`) for composition, FRONTIER for multi-city planning + the corrective critic check. **OpenAI is the primary provider**; Anthropic and Groq are configured as fallback rungs and stay inert until their key is set, so the chain self-prunes. Prompt caching (OpenAI's automatic prefix cache, read back via `cached_tokens`) is the primary cost lever. Model IDs and prices are config — re-verify against each provider's catalog.
- **Reasoning:** Quality-first = frontier on quality-critical steps, cheap on routine fan-out. Caching cuts repeated context to a fraction of full input price. A single provider can't underwrite a 99.5% SLO, so cross-provider fallback is load-bearing, not decorative. The gateway is an anti-corruption layer: swapping the primary provider is a routing-table change, not a rewrite.
...
- **Reasoning:** A multi-agent planner's transparency *is* the product — users trust a grounded plan when they can watch it being built and click through to sources. Next.js gives first-class SSE streaming plus the map and auth integrations that needs; Streamlit can't render a live agent trace credibly and isn't a client-facing surface.
...
- **Reasoning:** Mature and battle-tested, and it reuses the Redis already in the stack. Keep tasks thin so the async↔Celery bridge stays simple.
- **Reversibility:** Moderate.
...
- **Decision:** **OpenTelemetry** (traces/metrics/logs) through FastAPI + Celery + the agent; **Prometheus + Grafana** (metrics); **Loki** (structured JSON to STDOUT); **Langfuse** for agent/LLM tracing (LangSmith = native-LangGraph alt; ELK = alt if a client already runs it). Alertmanager → Slack/PagerDuty. Trace: plan → per-city sub-agent tool calls → tokens/cost/latency/tier/cache → critic verdict + correction rounds + partial flags. SLO/error-budget alerts on runaway runs, tool-error spikes, p95 breach.
- **Reasoning:** Multi-agent runs fail silently and expensively without per-step tracing, so observability is a first-class requirement here rather than an add-on. OTel keeps the backend swappable; Langfuse adds LLM-native views (generations, cost) on the same spans.
...
- **Decision:** **Docker multi-stage / non-root** (healthcheck + .dockerignore) → **AWS EKS.** Terraform modular (`vpc/eks/data/app`); Helm charts; ArgoCD GitOps. ECS Fargate = simpler swap.
- **Cost reality:** an always-on EKS cluster (~$73/mo control plane + nodes) breaks ≤$50/mo. Mitigation: dev on local kind/minikube; stand EKS up for staging/prod demo + the k6 load test; scale nodes to ~zero / tear down between sessions. EKS is the demonstrated target, not 24/7 idle.
- **Reasoning:** EKS gives real node-pool control, cluster autoscaling, and multi-AZ scheduling — the things a single-node local cluster can't exercise. Helm + ArgoCD keep the deployment declarative and reviewable.
...
- **Reasoning:** The industry default for a GitHub-hosted repo, with no runner to operate. The mandatory eval gate is what separates a deploy pipeline from a build script.
- **Reversibility:** Easy-Moderate.

### Decision 17: Secrets & configuration
- **Options:** **Secrets Manager + ESO + pydantic-settings** / k8s Secrets only / Vault.
- **Decision:** **AWS Secrets Manager** (KMS + rotation) as source of truth; **External Secrets Operator** syncs to K8s; **pydantic-settings** typed config that **fails fast on a missing key** (never a silent `None` reaching a provider call). `.env` local-dev only (gitignored; commit `.env.example`). Vault = future upgrade.
...
- **Reasoning:** Eval is what separates a demo from a system you can trust in production, and multi-agent makes trajectory eval non-optional: a run can look successful while quietly skipping a city or inventing a place.
- **Reversibility:** Easy.
